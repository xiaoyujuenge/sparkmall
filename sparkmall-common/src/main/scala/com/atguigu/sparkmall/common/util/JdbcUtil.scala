...
object JdbcUtil {
        ...
        def executeUpdate(sql: String, params: Array[Any]): Int = { // "insert into xxx values (?,?,?)"
            var rtn = 0
            //PreparedStatement 预编译器,编译sql语句
        ...
        def main(args: Array[String]): Unit = {
            JdbcUtil.executeUpdate("insert into category_top10 values(?,?,?,?,?)", Array("task010","99",100,200, 12 ) )
            JdbcUtil.executeUpdate("insert into category_top10 values(?,?,?,?,?)",Array("task010","99",102,200, 12))
        }
